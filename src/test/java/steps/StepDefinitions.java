...
public class StepDefinitions {
	...
	@Given("user launch app using {string}")
	public void user_launch_app_using(String url) {
	...
	@When("user clicks button xpath {string}")
	public void user_clicks_button_xpath(String xpath) {
	...
	public void user_verify_the_presence_of_text_using_xpath(String xpath, DataTable dataTable) {
		...
		List<Map<String,String>> listogMaps=dataTable.asMaps();
		
		String actualText =driver.findElement(By.xpath(xpath)).getText();
		
		System.out.println(actualText);
		
		System.out.println(listogMaps.get(0).get("error message"));
		
		Assert.assertEquals(listogMaps.get(0).get("error message"), actualText);
	}
